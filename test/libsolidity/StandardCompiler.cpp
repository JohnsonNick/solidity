...
void expectLinkReferences(Json::Value const& _contractResult, map<string, set<string>> _expectedLinkReferences)
{
...
BOOST_AUTO_TEST_CASE(linking_yul_same_library_name_different_files)
{
	char const* input = R"(
	{
		...
		"sources": {
			"fileA": {
				"content": "object \"a\" { code { let addr1 := linkersymbol(\"fileB:L\") let addr2 := linkersymbol(\"fileC:L\") } }"
			}
		}
	}
	)";
